...
class FacebookTabBar extends React.Component {
  ...
  setAnimationValue({ value }) {
    ...
    this.lines.forEach((line, i) => {
      const progress = (value - i >= 0 && value - i <= 1) ? value - i : 1;
      line.setNativeProps({
        style: { backgroundColor: this.getColor(this.props.pixel, progress)}
      });
    });
  }

  render() {
    return <View style={[styles.tabs, this.props.style, {backgroundColor: `rgb(${this.props.pixel},${this.props.pixel},${this.props.pixel})`}]}>
      {this.props.tabs.map((tab, i) => {
        return <TouchableOpacity key={tab} onPress={() => this.props.goToPage(i)} style={styles.tab}>
          <View style={styles.tabWarp}>
            ...
            <View ref={(line) => { this.lines[i] = line; }} style={[styles.line, {
              height: this.props.activeTab === i ? 2 : 0
            }]}></View>
          </View>
        </TouchableOpacity>;
      })}
